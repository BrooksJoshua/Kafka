...
## show profile (查询剖析)
要使用`profiling`功能, 首先确保其已开启. 该session变量:profiling默认是关闭OFF(0), 可以通过set指令进行开启.
```mysql
show variables like '%profil%'
```
开启
```mysql
set profiling ='ON'; -- 等号右边也可以是1, (0代表关闭, 1代表开启)
```
![开启profiling](MySQL调优/开启profiling.png)

官网给出的`show profile` 指令格式:
```mysql
...
`show profiles`: 注意带s, 该指令会返回最近一批发送到服务端的查询语句的信息(包含query_id, duration, statements),具体返回多少条由`profiling_history_size`变量决定. 其默认值是15, 上限100. 如果设置为0, 等效于: `set profiling ='OFF';`  

MySQL5.7版本官网提示, show profile有可能不会在未来版本中出现,  取而代之的将会是 `performance schema`.

**The SHOW PROFILE and SHOW PROFILES statements are deprecated and will be removed in a future MySQL release. Use the Performance Schema instead; see [Section 25.19.1, “Query
 Profiling Using Performance Schema”](https://dev.mysql.com/doc/refman/5.7/en/performance-schema-query-profiling.html).** 
```mysql
show profile all for query 1; -- 查询性能详细信息
```
**如果不指明for query n, 则返回最近一条查询语句的性能分析.**
...
``` 
![show_processlist](MySQL调优/show_processlist.png)
指令的输出列[官网解释译文](https://dev.mysql.com/doc/refman/5.7/en/show-processlist.html):
<ol>
